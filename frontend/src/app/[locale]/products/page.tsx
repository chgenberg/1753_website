    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="pt-20 pb-16">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
              <div className="flex flex-wrap items-center gap-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {categories.map(category => (
                    <button key={category} onClick={() => setSelectedCategory(category)} className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 ${selectedCategory === category ? 'bg-[#4A3428] text-white shadow-lg' : 'bg-white text-gray-700 border border-gray-200 hover:border-[#4A3428] hover:text-[#4A3428]'}`}>
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="relative">
                  <select value={sortBy} onChange={(e) => setSortBy(e.target.value as any)} className="appearance-none bg-white border border-gray-200 rounded-lg px-4 py-2 pr-8 text-sm focus:outline-none focus:ring-2 focus:ring-[#4A3428] focus:border-transparent">
                ...
                {selectedCategory !== 'alla' && (
                  <button onClick={resetFilters} className="text-sm text-[#4A3428] hover:text-[#3A2418] font-medium transition-colors">
          ...
          <AnimatePresence mode="wait">
            {filteredProducts.length === 0 ? (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-16">
                ...
                <button onClick={resetFilters} className="mt-4 px-6 py-2 bg-[#4A3428] text-white rounded-lg hover:bg-[#3A2418] transition-colors">
                ...
                {filteredProducts.map((product, index) => (
                  <motion.div key={product.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: index * 0.1 }} className="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300 group relative" onMouseEnter={() => setHoveredProduct(product.id)} onMouseLeave={() => setHoveredProduct(null)}>
                    <div className="absolute top-4 left-4 z-10 flex flex-col gap-2">
                      {product.featured && (<span className="bg-[#4A3428] text-white text-xs font-bold px-3 py-1 rounded-full">{t('productsPage.badges.recommended')}</span>)}
                      {product.bestseller && (<span className="bg-[#8B4513] text-white text-xs font-bold px-3 py-1 rounded-full">{t('productsPage.badges.bestseller')}</span>)}
                    ...
                    <div className="p-6">
                      <div className="mb-2"><span className="text-xs font-medium text-[#4A3428] uppercase tracking-wider">{product.category}</span></div>
                      <h3 className="text-lg font-bold text-gray-900 mb-2 group-hover:text-[#4A3428] transition-colors">{product.name}</h3>
                      ...
                      <button onClick={() => addToCart({ ...product, tags: [], images: [{ url: product.image, alt: product.name, position: 0 }], variants: [], inventory: 100, trackInventory: false, allowBackorder: true, isActive: true, isFeatured: product.featured, seoKeywords: [], keyIngredients: [], skinConcerns: [], timeOfDay: undefined, benefitsDetails: [], ingredientsDetails: [], imagesData: [], rating: product.rating || { average: 0, count: 0 }, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() })} className="w-full bg-[#4A3428] text-white py-3 rounded-xl font-medium hover:bg-[#3A2418] transition-colors flex items-center justify-center gap-2 group">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="mt-16 bg-gradient-to-r from-[#4A3428] to-[#3A2418] rounded-2xl p-8 text-center text-white">
            ...
            <div className="max-w-md mx-auto flex gap-4">
              ...
              <button className="bg-white text-[#4A3428] px-6 py-3 rounded-lg font-medium hover:bg-gray-100 transition-colors">{t('productsPage.newsletter.subscribe')}</button>
            </div>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.2 }} className="mt-8 text-center">
            <Link href="/quiz" className="inline-flex items-center gap-2 bg-white border-2 border-[#4A3428] text-[#4A3428] px-8 py-4 rounded-xl font-medium hover:bg-[#4A3428] hover:text-white transition-all duration-300 shadow-lg">

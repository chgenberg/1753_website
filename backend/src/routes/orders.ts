...
router.post('/create', optionalAuth, async (req: any, res) => {
  try {
    ...
    const paymentOrderParams: any = {
      ...
      successUrl: `${process.env.FRONTEND_URL || 'https://1753website-production.up.railway.app'}/checkout/success?orderCode=${order.orderNumber}`,
      cancelUrl: `${process.env.FRONTEND_URL || 'https://1753website-production.up.railway.app'}/checkout?cancelled=1`,
